  ...
  <div class="journal-submission-container">
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <h2 class="page-title">Nature Genetics 投稿指南</h2>
    ...
    <div class="main-content">
      ...
      <div class="result-section">
        ...
        <div v-if="isLoading && !submissionGenerated" class="loading-result">
          ...
          <p class="loading-text">AI正在生成Nature Genetics初稿...</p>
  ...
  setup() {
    ...
    const generateSubmission = async () => {
      ...
      try {
        ...
        formData.append('theme', 'sci_paper');
    ...
    const markdownToHtml = (markdownText: string): string => {
      if (!markdownText || typeof markdownText !== 'string') {
        return markdownText || ''; // 如果输入无效，返回空字符串或原始值
      }
      // 检查是否已经是HTML (简单检查)
      if (markdownText.includes('<') && markdownText.includes('>')) {
        // 如果包含HTML标签，则尝试查找特定的Markdown标记，如果找不到，则假设已经是HTML
      ...
      // 处理段落 (任何没有被其他规则处理的行都视为段落)
      // 首先用换行符分割，然后过滤掉已经被转换成HTML块级元素的行
      const lines = processedText.split(/\r?\n/);
      let inHtmlBlock = false;
      const resultLines = lines.map(line => {
        const trimmedLine = line.trim();
        if (trimmedLine.startsWith('<') && trimmedLine.endsWith('>')) {
           // 简单判断是否是HTML块级标签的开始或结束
          if (trimmedLine.match(/^<(div|p|h[1-6]|ul|ol|li|table|thead|tbody|tr|th|td|blockquote|pre|hr)/i) || 
            ...
            return line; // 已经是HTML块级元素或者在其内部，直接返回
          }
        }
        if(inHtmlBlock && !trimmedLine.endsWith('</pre>')) return line; //在pre内部的代码行
    ...
    const downloadContent = () => {
      ...
      try {
        ...
        const wordContent = `
          ...
          <head>
          <meta charset="utf-8">
          <title>${paperInfo.value || 'Nature Genetics 投稿指南'}</title>
          </head>
          <body>
          ${generatedSubmission.value}
          </body>
        ...
        // 如果标题为空，使用默认名称
        if (!fileName) {
          fileName = `Nature_Genetics_投稿指南_${new Date().toISOString().split('T')[0]}`;
        }
        ...
        ElMessage.success('投稿指南已下载为Word文档');
      } catch (error) {
